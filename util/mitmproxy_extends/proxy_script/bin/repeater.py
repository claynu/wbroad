...
def restore_res(tmp_list):
    ...
    req_id, url, req, token, length, resp, resp_headers, resp_status, check_type, role_type, risk_type = tmp_list
    # todo 部分请求入库失败
    con = con_sqlite()
    cursor = con.cursor()
    try:
        sql = f"insert into response(req_id,url,req,token,length,resp,resp_headers,resp_status,check_type,role_type, risk_type) values('{req_id}','{url}','{req}','{json.dumps(token)}','{length}','{resp}','{resp_headers}','{resp_status}','{check_type}','{role_type}','{risk_type}')"
        cursor.execute(sql)
...
def repeater(record: tuple, auth_dict: dict):
    ...
    for auth_type in auth_dict:
        ...
        try:
            if method == 'POST':
                response = requests.post(url=url, headers=headers, json=json.loads(body), proxies=proxy, verify=False)
            if method == 'GET':
            ...
            restore_res_param_temp_list.append([id, url, json.dumps(req), auth_dict[auth_type], length, resp,
...
if __name__ == '__main__':
    ...
    proxy = config['target']['auth']['proxy']
    if not proxy.get('enable', False):
        proxy = None
