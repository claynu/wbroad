    ...
    def write(self, value, status=CONTENT_STATUS.IN_PROGRESS, content_type=None):
        if self.messagetype == "stdout":
            ...
            output = conf.databaseCursor.execute(
                "SELECT id, status, value FROM data WHERE taskid = ? AND content_type = ?", (self.taskid, content_type))
...
def createTasks():
    if request.method == 'POST':
        ...
        print(data['file'])
        file = data['file']
        # file = data['file'].encode().decode('unicode_escape')
        print(file)
        ...
        # 初始化task
        DataStore.tasks[taskid] = Task(taskid, remote_addr)
        t = Task(taskid, remote_addr)
        # 添加到project对象中
        if project not in DataStore.projects.keys():
            DataStore.projects[project] = []
        DataStore.projects[project].append(taskid)
        ...
        DataStore.tasks[taskid].set_option('requestFile', file)
        DataStore.tasks[taskid].engine_start()
        return jsonize({'tasks': [taskid]})
    else:
        return "<h1>只能用POST哈</h1>"


@get("/getResult/<taskid>")
def getResult(taskid=None):
    result_detail = []
    result = 'Failed'
    if taskid is not None:
        host, uri, project = DataStore.infos[taskid]
        status = dejsonize(scan_status(taskid)).get('status','')
        if status == 'running':
            return {'project': project, 'taskid': taskid, 'host': host, 'uri': uri, 'status': status, 'result_detail': 'running','result':'None'}
        for message in DataStore.current_db.execute(
                "SELECT message FROM logs WHERE taskid = ? and level = ? ", (taskid, 'CRITICAL',)):
            if isinstance(message, tuple) or isinstance(message, list):
                message = message[0]
            if message.find('all tested parameters do not appear to be injectable') != -1:
                return {'project': project, 'taskid': taskid, 'host': host, 'uri': uri, 'status': 'terminated',
                        'result_detail': 'could not find the sql injection', 'result': 'Pass'}
            return {'project': project, 'taskid': taskid, 'host': host, 'uri': uri, 'status': 'terminated',
                    'result_detail': 'terminated by Exception, please check log.', 'result': 'Exception'}

        for message in DataStore.current_db.execute(
                "SELECT message FROM logs WHERE taskid = ? ORDER BY id DESC LIMIT ? ", (taskid, 40,)):
            print(message)
        for status, content_type, value in DataStore.current_db.execute(
            ...
            result_detail.append({"status": status, "type": content_type, "value": dejsonize(value)})
        if result_detail == []:
            return {'project': project, 'taskid': taskid, 'host': host, 'uri': uri, 'status': 'terminated',
                    'result_detail': 'terminated by User, please check log.', 'result': 'Exception'}
        result_detail, url = format_result_detail(str(result_detail))
        return {'project': project, 'taskid': taskid, 'host': host, 'uri': uri,'status': 'terminated', 'detail': result_detail,'result':result}
    return None

def format_result_detail(result_detail:str):
    result_detail = result_detail.replace('", \'',"', '")
    result_detail = result_detail.replace("'payload': \"", "'payload': '")
    url = re.findall(r"'url': '(.*?)',",result_detail)
    url = '' if url == [] else url[0]
    return result_detail, url


@get("/getLogs/<taskid>")
def getLogs(taskid=None):
    data = scan_log(taskid)
    return data
...
def getLogs(taskid=None):
    scan_stop(taskid)
    data = task_delete(taskid)
    for project,tasks in DataStore.projects.items():
        if taskid in tasks:
            tasks.remove(taskid)
    return data
...
@get("/getTaskList")
def getTaskList():
    project = base64.urlsafe_b64decode(request.params['project']).decode('utf-8')
    taskids = DataStore.projects.get(project,[])
    res = {}
    for i in range(0, len(taskids)):
        res[taskids[i]] = getResult(taskid=taskids[i])
        res[taskids[i]]['project'] = project
    print(res)
    return json.dumps(res, ensure_ascii=False)

path = os.environ.get('APPDATA')+os.path.sep+'sql_log'
if not  os.path.exists(path):
    os.makedirs(path)

def find_info(data):
    host = re.findall('Host:(.*?)\n',data)
    host = '' if host is [] else host[0].strip()
    uri = re.findall(' (.*?) HTTP/',data)
    uri = '' if uri is [] else uri[0].strip()
    return host, uri
...
def createTasks():
    if request.method == 'POST':
        ...
        try:
            if data['request'] == '':
                return jsonize({'error': '请求不能为空!'})
            content = base64.b64decode(data['request'].encode('utf-8'))
            host, uri = find_info(content.decode('utf-8'))
        except IndexError as e:
            print('请求解码失败')
            return jsonize({'error': '请求解码失败!'})
        file_name = path+os.path.sep+'sqli_det_'+str(time.time())
        print(file_name)
        ...
        if options == '':
            options = None
        project = data['project']
        project = base64.urlsafe_b64decode(project).decode('utf-8')

        taskid = encodeHex(os.urandom(8), binary=False)
        remote_addr = request.remote_addr
        # 初始化task
        DataStore.tasks[taskid] = Task(taskid, remote_addr)
        DataStore.infos[taskid] = (host,uri,project)
        t = Task(taskid, remote_addr)
        # 添加到project对象中
        if project not in DataStore.projects.keys():
            DataStore.projects[project] = []
        ...
        for option, value in options.items():
            print(option, value)
            DataStore.tasks[taskid].set_option(option, value)
        DataStore.tasks[taskid].set_option('requestFile', file_name)
        DataStore.tasks[taskid].engine_start()
        return jsonize({'tasks': [taskid]})
    else:
        return "<h1>只能用POST哈</h1>"
...
def option_list(taskid):
    ...
    # print(DataStore.tasks[taskid].get_options())
...
def scan_stop(taskid):
    ...
    if (taskid not in DataStore.tasks or DataStore.tasks[taskid].engine_process() is None or DataStore.tasks[
        taskid].engine_has_terminated()):
...
@get("/scan/<taskid>/log/<length>")
def scan_log(taskid,length=100):
    """
    ...
    for time_, level, message in DataStore.current_db.execute(
            "SELECT time, level, message FROM logs WHERE taskid = ? ORDER BY id DESC LIMIT ?", (taskid,length,)):
        json_log_messages.append({"time": time_, "level": level, "message": message})
...
def server(host=RESTAPI_DEFAULT_ADDRESS, port=RESTAPI_DEFAULT_PORT, adapter=RESTAPI_DEFAULT_ADAPTER, username=None,
           password=None):
    """
    ...
    try:
        ...
        if adapter.lower() not in server_names:
            ...
            errMsg += "(Note: you can try to install it with 'apt install python-%s' or 'pip%s install %s')" % (
            adapter, '3' if six.PY3 else "", adapter)
    ...
    try:
        ...
        if DataStore.username or DataStore.password:
            headers["Authorization"] = "Basic %s" % encodeBase64(
                "%s:%s" % (DataStore.username or "", DataStore.password or ""), binary=False)
...
def client(host=RESTAPI_DEFAULT_ADDRESS, port=RESTAPI_DEFAULT_PORT, username=None, password=None):
    ...
    commands = (
    "help", "new", "use", "data", "log", "status", "option", "stop", "kill", "list", "flush", "version", "exit", "bye",
    "quit")
    colors = ('red', 'green', 'yellow', 'blue', 'magenta', 'cyan', 'lightgrey', 'lightred', 'lightgreen', 'lightyellow',
